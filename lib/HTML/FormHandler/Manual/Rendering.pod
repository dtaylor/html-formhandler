...
a 'render' method already exists, so if your form class applies a role like:
   ...
   with 'HTML::FormHandler::Render::MyRenderer';

then L<HTML::FormHandler::Render::Simple> will not be applied.
...
The following attributes can be set on a form:

   widget_name_space
   widget_wrapper
   widget_form

The following attributes can be set on a field:
